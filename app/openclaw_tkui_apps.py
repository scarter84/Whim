...
class ModernApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Enoch — Bones")
        self.geometry("1280x800")

        # Load Azure theme (you already have the files)
        script_dir = os.path.dirname(os.path.abspath(__file__))
        azure_tcl = os.path.join(script_dir, "azure.tcl")
        if os.path.exists(azure_tcl):
            self.tk.call("source", azure_tcl)
            self.tk.call("set_theme", "dark")
            print("✅ Azure dark theme loaded successfully!")
        else:
            print("⚠️ azure.tcl not found - using default look")
    ...
    def build_ui(self):
        ...
        ttk.Label(header, text="🦾 Enoch — OpenClaw", font=("Segoe UI", 11, "bold")).pack(side="left", padx=10)
    ...
    def build_xtts(self):
        ...
        ttk.Label(f, text="Text to speak:").pack(anchor="w", padx=12)
    ...
    def xtts_test(self):
        ...
        try:
            ...
            self.xtts_out.insert("end", f"✅ Status: {r.status_code} | {len(r.content)} bytes received\n\n")
        except Exception as e:
            self.xtts_out.insert("end", f"❌ Error: {e}\n")

    def build_signal(self):
        ttk.Label(self.tabs["signal"], text="Signal setup coming soon…", font=("Segoe UI", 12)).pack(expand=True)
    ...
    def on_connect(self):
        ...
        self.log_events("🔌 Connecting to gateway...")
